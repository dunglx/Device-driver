...
static struct spi_board_info spi_plat_board[] __initdata = {
	[0] = {
		...
		.max_speed_hz    = 4000000,     /* max spi clock (SCK) speed in HZ 3125000*/
...
/*----- DungLX: led-sysfs driver -------*/
#if defined(CONFIG_LEDS_GPIO_SYSFS)
...
static struct platform_device led_sysfs_device = {
	.name		= "led-sysfs",
	.id 		= -1,
	.dev		= {
		.platform_data = &led_sysfs_pdata,
	},
}
#endif 
/*---------------------------------------
...
/*---- DungLX dang ki led_sysfs device -----*/
#if defined
	printk("DungLX: add device led_sysfs\n");
	platform_device_register(&led_sysfs_device);
/* ------------------------------------------*/
	/* END */
	printk("\n");
}
